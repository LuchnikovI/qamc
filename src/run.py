...
def run(cfg):
    ...
    if couplings_ensemble_params is None:
        couplings_ensemble_params = dict()
    else:
        couplings_ensemble_params = {str(k): float(v) for k, v in couplings_ensemble_params.items()}
    ...
    if local_fields_ensemble_params is None:
        local_fields_ensemble_params = dict()
    else:
        local_fields_ensemble_params = {str(k): float(v) for k, v in local_fields_ensemble_params.items()}
    ...
    for sn in spins_number:
        for t in tau:
            for g in gamma:
                ...
                log.info(f"Finished: spins_number: {sn}, tau: {t}, gamma: {g}")
